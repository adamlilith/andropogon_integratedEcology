...
###
### This model estimates the distribution of site-level mean Andropogon gerardi ramet biomass. The distribution of biomasses among ramets at a site follows a gamma distribution with individual ramet biomasses drawn from this distribution. The site-level mean biomass is a function of soil/climate.
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm_for_sem/sem_04a_model_biomass~gamma.r')
...
###########################
	...
	out_dir <- paste0('./outputs_loretta/sdm_pdm_for_sem/', ifelse(trial, 'TRIAL_', ''), 'model_biomass~gamma~', preds_filename, '/')
...
#############
	...
	say('This model estimates the distribution of site-level mean Andropogon gerardi ramet biomass. The distribution of biomasses among ramets at a site follows a gamma distribution with individual ramet biomasses drawn from this distribution. The site-level mean biomass is a function of soil/climate.', breaks = 60, post = 1)
	...
	model_code <- nimbleCode({
		...
		for (i in 1:n_biomass) {
			...
			# likelihood
			y[i] ~ dgamma(shape = shape[site_index_biomass[i]], rate = rate[site_index_biomass[i]])

			# simulated values for unconditional DHARMa residuals
			y_sim[i] ~ dgamma(shape = shape[site_index_biomass[i]], rate = rate[site_index_biomass[i]])
