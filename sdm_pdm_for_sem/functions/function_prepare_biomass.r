...
prepare_biomass <- function(formula_biomass, n_response_curve_values = 200, calib = TRUE) {
	...
	biomass_data_raw$site_soil_pH <- soil_data_raw$pH[matches]
	biomass_data_raw$site_soil_pH_sikora <- soil_data_raw$`Sikora_pH`[matches]
	biomass_data_raw$site_soil_N <- soil_data_raw$`Total.N.%`[matches]
	...
	site_biomass_mean <- mean(biomass_data_raw[ , .(val = mean(Biomass)), by = SITE][['val']])
	site_biomass_sd <- mean(biomass_data_raw[ , .(val = sd(Biomass)), by = SITE][['val']])
	...
	ag_vect_sq <- vect('./outputs_loretta/sdm_pdm_for_sem/andropogon_gerardi_occurrences_with_environment_1961_2020_for_integration.gpkg')
	...
	for (clim in c('thirties', 'fifties')) {
		...
		assign(paste0('counties_x_biomass_', clim), mm)
	...
	list(
		...
		terms_biomass = terms,					# terms in formula_biomass
		covariates_biomass = covariates,		# covariates in formula_biomass (which can appear in >1 term)
		n_terms_biomass = n_terms,				# number of terms in formula_biomass
		n_covariates_biomass = n_covariates,	# number of variables in formula_biomass (which can appear in >1 term)
		...
		site_biomass_mean = site_biomass_mean,	# mean site-level biomass
		site_biomass_sd = site_biomass_sd,	# mean of site-level biomass s.d.

		x_by_site_biomass = x_by_site,			# MM with covariates (scaled)
		x_centers_biomass = x_centers,			# vector of means for each covariate
		x_scales_biomass = x_scales,			# vector of standard deviations for each covariate

		site_vect_biomass = site_vect,			# SpatVector of site data for plotting
		...
		# counties_x_biomass_sq_calib = counties_x_sq_calib, # MM of county-level environmental data for present (SDM calibration region)
		counties_x_biomass_sq = counties_x_sq,		# MM of county-level environmental data for current conditions
		counties_x_biomass_ssp245_2041_2070 = counties_x_ssp245_2041_2070, # MM of county-level environmental data for future conditions (SSP245, 2041-2070)
		counties_x_biomass_ssp245_2071_2100 = counties_x_ssp245_2071_2100, # MM of county-level environmental data for future conditions (SSP245, 2071-2100)
		counties_x_biomass_ssp370_2041_2070 = counties_x_ssp370_2041_2070, # MM of county-level environmental data for future conditions (SSP370, 2041-2070)
		counties_x_biomass_ssp370_2071_2100 = counties_x_ssp370_2071_2100, # MM of county-level environmental data for future conditions (SSP370, 2071-2100)
