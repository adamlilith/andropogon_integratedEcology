...
say('#############################################################################################')
	...
	aridity <- (climatena$bio1 + 10) / (climatena$bio12 / 1000)
	names(aridity) <- 'aridity'
	climatena <- c(climatena, aridity)
	...
	sq_lambda_vect <- vect(paste0('./outputs_loretta/sdm_[nmixture]_[pseudoabsences_0.99]_[bio1^2_bio12^2_bio15^2]_[priors_ddnorm]/sdm_nmixture_1961_2020_climate_focus.gpkg'))
	sq_env_vect <- vect(paste0('./data_from_adam_and_loretta/andropogon_gerardi_occurrences_with_environment_1961_2020_climatena.gpkg'))
	vars <- sort(unique(c(MASTER_climate_predictor_names, 'lambda_mean')))
	if (exists('sq_rasts')) rm(sq_rasts)
	for (var in vars) {

		if (var %in% names(sq_lambda_vect)) {
			sq_vect <- sq_lambda_vect
		} else if (var %in% names(sq_env_vect)) {
			sq_vect <- sq_env_vect
		}
	...
	for (fut in futs) {
		...
		fut_vect <- vect(paste0('./outputs_loretta/sdm_[nmixture]_[pseudoabsences_0.99]_[bio1^2_bio12^2_bio15^2]_[priors_ddnorm]/sdm_nmixture_', fut, '.gpkg'))

		this_vars <- vars[vars %in% names(fut_vect)]
