...
say(date(), post = 1)
	...
	x_sq_complement <- as.data.frame(x_sq_complement[ , c('area_km2', predictor_names)])
...
say('###################################')
	...
	map <- ggplot() +
		...
		ggtitle(expression('Present-day distribution of ' * italic('Andropogon gerardi')), subtitle = '1961-2020') +
		theme(
...
say('###################################')
	...
	for (fut in futs) {
		...
		pretty_title <- paste0('SSP ', substr(fut, 4, 6), ' ', substr(fut, 8, 11), '-', substr(fut, 13, 16))
