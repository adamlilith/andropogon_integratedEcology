...
say(date(), post = 1)
	...
	n_poaceae <- ag_vect_sq$n_poaceae
	n_pseudobsences <- quantile(n_poaceae, probs = psa_quant, na.rm = TRUE)
	ag_vect_sq$any_ag_quality1to3[nas] <- 0
	ag_vect_sq$num_ag_quality1to3[nas] <- 0
	ag_vect_sq$n_poaceae[nas] <- n_pseudobsences
	...
	log_num_poaceae_records <- log1p(ag_sq$n_poaceae) # log(x + 1)
