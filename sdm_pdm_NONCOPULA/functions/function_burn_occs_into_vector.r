...
burn_occs_into_vector <- function(demesne, chains, formula_occs, formula_psi = NULL) {
	...
	pred_vect$DUMMY2 <- preds_inner_quant

	index <- (ncol(pred_vect) - 1):ncol(pred_vect)
	if (demesne == 'nam') {
		names(pred_vect)[index] <- c('N_ag_county_mean_sq', 'N_ag_county_inner_quant_sq')
	} else if (demesne == '1930s') {
		names(pred_vect)[index] <- c('N_ag_county_mean_1930s', 'N_ag_county_inner_quant_1930s')
	}
	...
	if (demesne == 'nam') {
		...
		for (fut in futs) {
			...
			index <- (ncol(pred_vect) - 1):ncol(pred_vect)
			names(pred_vect)[index] <- paste0(c('N_ag_county_mean_', 'N_ag_county_inner_quant_'), fut)
