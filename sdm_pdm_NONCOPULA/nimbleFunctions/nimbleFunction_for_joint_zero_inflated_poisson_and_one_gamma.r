...
#' @param psi Numeric >=0 and <= 1. Parameter of gamma distribution: Probability of 0's.
...
dzipoisgamma1 <- nimbleFunction(
    ...
    if (x1 == 0 & x2 == 0) {
        ...
        dens <- psi + (1 - psi) * dpois(0, lambda, log = 0)
    } else if (x1 > 0 & x2 > 0) {
		dens <- (1 - psi) * dpois(x1, lambda, log = 0) * dgamma(x2, shape = shape, rate = rate, log = 0)
