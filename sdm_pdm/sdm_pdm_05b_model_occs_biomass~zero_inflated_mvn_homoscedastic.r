...
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_05b_model_occs_biomass~zero_inflated_mvn_homoscedastic.r')
### 
#############
### setup ###
#############
...
###########################
	...
	pzero_filename <- 'bio1^2_bio12^2_bio15^2'

	### output folder and bias formula
	out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occs_biomass/', ifelse(trial, 'TRIAL_', ''), '[occs~zip[', occs_filename, '_bias~', bias_filename, ']]_[biomass~', biomass_filename, ']_[pzero_', pzero_filename, ']_homoscedastic/')
...
#############
	...
	data_occs_as_sites <- prepare_biomass(formula_biomass = formula_occs, n_response_curve_values = n_response_curve_values, calib = calib)

	# data for biomass using county-level environment
	data_biomass_as_counties <- prepare_occurrences(formula_occs = formula_biomass_mu, formula_occs_bias = ~ 1, n_response_curve_values = n_response_curve_values, psa_quant = psa_quant, calib = calib)

	# data for pzero at counties
	data_county_pzero <- prepare_occurrences(formula_occs = formula_pzero, formula_occs_bias = ~1, n_response_curve_values = n_response_curve_values, psa_quant = psa_quant, calib = calib)

	# data for pzero at sites
	data_site_pzero <- prepare_biomass(formula_biomass = formula_pzero, n_response_curve_values = n_response_curve_values, calib = calib)
	...
	constants <- list(
		...
		# zero inflation
		n_terms_pzero = data_county_pzero$n_terms_occs, # number of terms in sampling bias model
		x_by_county_pzero = data_county_pzero$counties_x_occs_sq,
		x_by_site_pzero = data_site_pzero$x_by_site_biomass
	...
	chains_occs <- readRDS('./outputs_loretta/integrated_sdm_pdm/models_occurrence_FAT/[occs_poisson~normal_homoscedastic_bio1^2_bio12^2_bio15^2_[bias~1]]/chains.rds')
	# alpha_occs_inits <- hammer_extract(chains_occs, 'alpha_occs', j = TRUE)
	# alpha_occs_inits <- hammer_extract(chains_occs, 'alpha_occs')
	alpha_occs_inits <- 0
	...
	inits <- list(
		...
		### integration
		eta = 1,
		Sigma_star = matrix(0.5, nrow = 2, ncol = 2),
		# log_sigma_multivar = rep(1, 2),
		# Ustar = matrix(1, nrow = 2, ncol = 2)
		Phi_county = matrix(1, nrow = data_occs$n_counties, ncol = 2),
		Phi_site = matrix(1, nrow = data_biomass_mu$n_pheno_sites, ncol = 2),

		### zero inflation
		beta_pzero = -1 * beta_occs_mu_inits
	...
	model_code <- nimbleCode({
		...
		# # INTEGRATION: covariance between occurrence and biomass
		eta ~ dgamma(2, 1)
		Sigma_star[1:n_facets, 1:n_facets] ~ dlkj_corr_cholesky(eta = eta, p = n_facets)  # LBJ prior for correlation matrix
		Sigma[1:n_facets, 1:n_facets] <- Sigma_star[1:n_facets, 1:n_facets] %*% t(Sigma_star[1:n_facets, 1:n_facets]) # Sigma = Sigma_star Sigma_star^T
		...
		for (i in 1:n_counties_occs_calib) {
			...
			# Phi_county[i, 1:n_facets] ~ dmnorm(phis_county[i, 1:n_facets], prec = precision_county[1:n_facets, 1:n_facets])
			Phi_county[i, 1:n_facets] ~ dmnorm(phis_county[i, 1:n_facets], cholesky = Sigma[1:n_facets, 1:n_facets], prec_param = 0)
			# Phi_county[i, 1:n_facets] ~ dmnorm(mean = phis_county[i, 1:n_facets], cholesky = U[1:n_facets, 1:n_facets], prec_param = 0)
		...
		for (i in 1:n_pheno_sites) {
			...
			# Phi_site[i, 1:n_facets] ~ dmnorm(phis_site[i, 1:n_facets], prec = precision_site[1:n_facets, 1:n_facets])
			Phi_site[i, 1:n_facets] ~ dmnorm(phis_site[i, 1:n_facets], cholesky = Sigma[1:n_facets, 1:n_facets], prec_param = 0)
			# Phi_site[i, 1:n_facets] ~ dmnorm(mean = phis_site[i, 1:n_facets], cholesky = U[1:n_facets, 1:n_facets], prec_param = 0)
		...
		for (i in 1:n_counties_occs_calib) {
			...
			N[i] ~ dzip(lambda_mu_sq[i], pzero = pzero_county[i])
			...
			logit(pzero_county[i]) <- inprod(beta_pzero[1:n_terms_pzero], x_by_county_pzero[i, 1:n_terms_pzero])
		...
		for (i in 1:n_covariates_occs) {
			...
			for (j in 1:n_response_curve_values) {
				...
				log(lambda_resp_curves_mu[j, i]) ~ dnorm(phi_lambda_resp_curves_mu[j, i], sd = Sigma[1, 1])
		...
		for (i in 1:n_pheno_sites) {
			...
			# site_biomass_mu_log[i] <- Phi_site[i, 2]
			...
			# probability of zero inflation
			logit(pzero_site[i]) <- inprod(beta_pzero[1:n_terms_pzero], x_by_site_pzero[i, 1:n_terms_pzero])
		...
		for (i in 1:n_biomass) {
			...
			# likelihood
			y_biomass[i] ~ dzigamma(shape = shape_biomass[site_index_biomass[i]], rate = rate_biomass[site_index_biomass[i]], pzero = pzero_site[site_index_biomass[i]])
	...
	monitors_coeffs_single_index <- c('beta_occs_mu', 'alpha_occs', 'beta_biomass_mu', 'beta_pzero')
	monitors_coeffs_double_index <- c('Sigma')
	# monitors_coeffs_double_index <- c('correlations')
	# monitors_coeffs_double_index <- c('Ustar')
...
say('#################################################')
	...
	descrip <- 'occurrence: zero-inflated Poisson ~ normal, zero-inflated biomass: gamma ~ normal, homoscedastic'
