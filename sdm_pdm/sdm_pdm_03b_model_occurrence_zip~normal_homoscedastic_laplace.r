...
###########################
	...
	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_zip~normal_homoscedastic_bio1^2_bio12^2_bio15^2_[bias~quad_ia]]', ifelse(trial, '_TRIAL', ''), '_laplace/')
	...
	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_zip~normal_homoscedastic_bio1^2_bio12^2_bio15^2_[bias~quad]]', ifelse(trial, '_TRIAL', ''), '_laplace/')
	...
	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_zip~normal_homoscedastic_bio1^2_bio12^2_bio15^2_[bias~ia]]', ifelse(trial, '_TRIAL', ''), '_laplace/')
	# formula_occs_bias <- ~ 1 + area_km2_log10 + n_poaceae_log10p1 + area_km2_log10:n_poaceae_log10p1 # sampling bias for AG records

	out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_zip~normal_homoscedastic_bio1^2_bio12^2_bio15^2_[bias~linear]]', ifelse(trial, '_TRIAL', ''), '_laplace/')
	formula_occs_bias <- ~ 1 + area_km2_log10 + n_poaceae_log10p1 # sampling bias for AG records

	# # out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_zip~normal_homoscedastic_bio1^2_bio12^2_bio15^2_[bias~1]]', ifelse(trial, '_TRIAL', ''), '_laplace/')
...
#############
	...
	constants <- list(
		...
		counties_x_occs_calib_sq = data_occs$counties_x_occs_sq # calibration data
	...
	# initial values for coefficients: occurrence ~ environment and bias
	...
	# initial values for coefficients: probability of absence
	prelim_data <- cbind(data_occs$w_occs_bias, data_occs$counties_x_occs_sq[ , 2:ncol(data_occs$counties_x_occs_sq)])
	y <- as.numeric(data_occs$y_n_ag > 0)
	prelim_model <- glm.fit(prelim_data, y = y, family = binomial())

	beta_occs_pzero_inits <- prelim_model$coefficients[c('(Intercept)', data_occs$terms_occs)]
	...
	# build <- buildLaplace(model, control = list(optimMethod = 'BFGS', trace = TRUE, maxit = 1000))
	build <- buildAGHQ(model, control = list(optimMethod = 'BFGS', trace = TRUE, maxit = 1000))

	say('Compiling Laplace ', date())
	# compiled_laplace <- compileNimble(build, project = model)
	compiled_hermite <- compileNimble(build, project = model)

	# laplace <- runLaplace(compiled_laplace)
	hermite <- runAGHQ(compiled_hermite)
	# saveRDS(laplace, paste0(out_dir, '/laplace.rds'))
	saveRDS(hermite, paste0(out_dir, '/hermite.rds'))
