...
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_03a_model_occurrence_poisson~normal_homoscedastic.r')
### 
#############
### setup ###
#############
...
###########################
	...
	# formula_occs <- ~ 1 + bio1 + bio12 + bio15 + silt + I(bio1^2) + I(bio12^2) + I(bio15^2) + I(silt^2) # response of occurrence to climate and soil
	# preds_filename <- 'bio1^2_bio12^2_bio15^2_silt^2'

	### output folder and bias formula

	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_poisson~normal_homoscedastic_', preds_filename, '_[bias~quad_ia]]', ifelse(trial, '_TRIAL', ''), '/')
	...
	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_poisson~normal_homoscedastic_', preds_filename, '_[bias~quad]]', ifelse(trial, '_TRIAL', ''), '/')
	...
	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_poisson~normal_homoscedastic_', preds_filename, '_[bias~ia]]', ifelse(trial, '_TRIAL', ''), '/')
	# formula_occs_bias <- ~ 1 + area_km2_log10 + n_poaceae_log10p1 + area_km2_log10:n_poaceae_log10p1 # sampling bias for AG records

	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_poisson~normal_homoscedastic_', preds_filename, '_[bias~linear]]', ifelse(trial, '_TRIAL', ''), '/')
	# formula_occs_bias <- ~ 1 + area_km2_log10 + n_poaceae_log10p1 # sampling bias for AG records

	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_poisson~normal_homoscedastic_', preds_filename, '_[bias~area]]', ifelse(trial, '_TRIAL', ''), '/')
	# formula_occs_bias <- ~ area_km2_log10 # sampling bias for AG records

	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_poisson~normal_homoscedastic_', preds_filename, '_[bias~area^2]]', ifelse(trial, '_TRIAL', ''), '/')
	# formula_occs_bias <- ~ area_km2_log10 + I(area_km2_log10^2) # sampling bias for AG records

	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_poisson~normal_homoscedastic_', preds_filename, '_[bias~poa]]', ifelse(trial, '_TRIAL', ''), '/')
	# formula_occs_bias <- ~ n_poaceae_log10p1 # sampling bias for AG records

	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_poisson~normal_homoscedastic_', preds_filename, '_[bias~poa^2]]', ifelse(trial, '_TRIAL', ''), '/')
...
#############
	...
	monitors_coeffs_single_index <- c('beta_occs_mu', 'alpha_occs')
