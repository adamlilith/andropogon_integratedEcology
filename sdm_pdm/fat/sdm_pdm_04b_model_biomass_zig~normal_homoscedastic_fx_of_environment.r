...
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_04b_model_biomass_zig~normal_homoscedastic_fx_of_environment.r')
...
###########################
	...
	out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_biomass/[biomass_zig~normal_homoscedastic_', preds_filename, ']', ifelse(trial, '_TRIAL', ''), '/')
...
#############
	...
	beta_biomass_pzero_inits <- -1 * beta_biomass_mu_inits
	...
	model_code <- nimbleCode({
		...
		for (i in 1:n_counties) {
			...
			log_biomass_county_mu_unscaled_sq[i] <-
