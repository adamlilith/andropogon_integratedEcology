...
###
### This script constructs a species distribution model Andropogon gerardi where "county" is the observational unit. It assumes (latent) abundance follows a Poisson distribution, and the observed number of AG is a binomial distribution where the probability of observing AG is an estimated constant value drawn from Unif(0, 1). The expected abundance drawn from a normal distribution where the mean value is given by a function of environmental predictors (climate, soil, etc.). The model is run using nimble.
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_03a_model_occurrence_poisson~normal_homoscedastic_bias~1_p~dunif.r')
### 
#############
### setup ###
#############
...
###########################
	...
	out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_poisson~normal_homoscedastic_', preds_filename, '_[bias~1_p~dunif]]', ifelse(trial, '_TRIAL', ''), '/')
...
#############
	...
	say('This script constructs a species distribution model Andropogon gerardi where "county" is the observational unit. It assumes (latent) abundance follows a Poisson distribution, and the observed number of AG is a binomial distribution where the probability of observing AG is an estimated constant drawn from Unif(0, 1). The expected abundance drawn from a normal distribution where the mean value is given by a function of environmental predictors (climate, soil, etc.). The model is run using nimble.', breaks = 60, post = 1)
	...
	inits <- list(
		...
		p = 0.5, # probability of detecting an individual
	...
	monitors_coeffs_single_index <- c('beta_occs_mu')
