...
###########################
	...
	out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occurrence/[occs_zip~normal_homoscedastic_', preds_filename, '_[bias~1]]', ifelse(trial, '_TRIAL', ''), '/')
...
#############
	...
	say('This script constructs a species distribution model Andropogon gerardi where "county" is the observational unit. It assumes (latent) abundance follows a zero-inflated Poisson distribution, and the observed number of AG is a binomial distribution where the probability of observing AG is aan estimated constant. The expected abundance drawn from a normal distribution where the mean value is given by a function of environmental predictors (climate, soil, etc.). The probability of a zero in the zero-inflation has the same functional form as the mean density model (but different coefficients). The model is run using nimble.', breaks = 60, post = 1)
	...
	# alpha_occs_inits <- hammer_extract(previous_chains, 'alpha_occs', j = TRUE, stat = 'mean') # if ~ 1, yields Inf likelihood
	...
	alpha_occs_init <- hammer_extract(previous_chains, 'alpha_occs', stat = 'mean')
	...
	inits <- list(
		...
		alpha_occs = 0, # intercept, area, # of Poaceae
