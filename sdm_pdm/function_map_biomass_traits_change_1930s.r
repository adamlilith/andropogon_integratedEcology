...
map_biomass_traits_change_1930s <- function(facet, data_biomass_traits, pred_vect_nam, pred_vect_conus) {
	...
	# change
	focal_vals <- unlist(pred_vect_conus[[paste0(facet, '_mu_county_thirties')]])
	sq_vals <- unlist(delta_vect[[paste0(facet, '_mu_county_sq')]])
	delta_vect$delta <- (focal_vals - sq_vals) / delta_vect$mu_biomass_county_sq
	
	if (zero_inflated) {

		focal_vals <- unlist(pred_vect_conus[[paste0('pzero_', facet, '_county_thirties')]])
