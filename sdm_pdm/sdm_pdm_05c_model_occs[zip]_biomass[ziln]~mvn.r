...
###########################
	...
	out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_occs_biomass/', ifelse(trial, 'TRIAL_', ''), '[occs~zip[', occs_filename, '_bias~', bias_filename, ']]_[biomass~ziln[', biomass_filename, ']]_[psi_', psi_filename, ']', ifelse(log_precip, '_log_precip', ''))
...
#############
	...
	chains_biomass <- readRDS(paste0('./outputs_loretta/integrated_sdm_pdm/models_biomass/[biomass_ziln~normal_',biomass_filename,  ']', ifelse(log_precip, '_log_precip', ''), '/chains.rds'))
	...
	model_code <- nimbleCode({
		...
		for (i in 1:n_biomass) {
			...
			# log_lik_biomass_y[i] <- dZILN(y_biomass[i], meanlog = site_biomass_mu[site_index_biomass[i]], sdlog = sigma_biomass_within_sites, z = z_site[site_index_biomass[i]])
...
say('#################################################')
	...
	descrip <- 'occurrence ~ ZIP(MVN), biomass ~ ZILN(MVN)'
