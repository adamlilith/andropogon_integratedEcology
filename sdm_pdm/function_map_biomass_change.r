...
map_biomass_change <- function(
	...
	vars <- c('mu_biomass_county_sq', 'mu_biomass_county_ssp245_2041_2070', 'mu_biomass_county_ssp245_2071_2100', 'mu_biomass_county_ssp370_2041_2070', 'mu_biomass_county_ssp370_2071_2100')

	min_val <- Inf
	max_val <- -Inf
	x_sq <- pred_vect_display$mu_biomass_county_sq[pred_vect_display$focal_region]
	for (var in vars) {
	...
	pred_vect_display$delta <- (pred_vect_display[[paste0('biomass_mu_county_', fut)]] - pred_vect_display$mu_biomass_county_sq) / pred_vect_display$mu_biomass_county_sq
	...
	if (plot_range_core) map <- map + layer_spatial(range_core, color = 'orange', fill = NA, size = 2)

	ggsave(plot = map, filename = paste0(out_dir, '/map_change_biomass_', filename_append, '.png'), width = 12, height = 10, dpi = 600)
	invisible(map)
