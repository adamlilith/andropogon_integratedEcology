...
#'
#' formula_occs, formula_psi, formula_bias Formulae for occurrences, probability of presence, and for occurrence bias
...
workflow_postmodeling_occurrence_crossvalidation <- function(formula_occs, formula_psi = NULL, formula_bias, constants, out_dir, overdispersed = FALSE) {
	...
	for (k in 1:max_k_folds) {
		...
		if (zero_inflated) fold_constants$counties_x_psi_calib_sq <- fold_constants$counties_x_psi_calib_sq[-indices, ]
		fold_constants$w_bias <- fold_constants$w_bias[-indices, ]
	
		test_x <- constants$counties_x_occs_calib_sq[indices, ]
		if (zero_inflated) test_x_psi <- constants$counties_x_psi_calib_sq[indices, ]
