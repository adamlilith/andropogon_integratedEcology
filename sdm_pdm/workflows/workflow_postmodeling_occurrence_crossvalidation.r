...
#'
workflow_postmodeling_occurrence_crossvalidation <- function(formula_occs, formula_occs_sigma,formula_occs_pzero, formula_occs_bias, constants, out_dir = out_dir) {
	...
	for (k in 1:max_k_folds) {
		...
		if (zero_inflated) fold_monitors <- c(fold_monitors, 'beta_occs_pzero')
