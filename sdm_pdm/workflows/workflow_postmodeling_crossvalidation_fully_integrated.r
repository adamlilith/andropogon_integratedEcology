...
#'
#' model_code			From nimbleCode()
#' nonbiomass_facets	List with metadata on non-biomass facets.
#' constants			`constants` `list` from model preparation.
#' out_dir				Folder into which to save results.
workflow_postmodeling_fully_integrated_crossvalidation <- function(
	model_code,
	nonbiomass_facets,
	constants,
	out_dir
	...
	cv_occs <- cv_biomass <- cv_nonbiomass <- data.table()
	for (k in 1:max_k_folds) {
		...
		fold_constants$x_by_site <- fold_constants$x_by_site[train_sites, ]
		
		if (zero_inflated) fold_constants$counties_x_psi_calib_sq <- fold_constants$counties_x_psi_calib_sq[-test_counties, ]
		fold_constants$w_bias <- fold_constants$w_bias[-test_counties, ]
		...
		x_by_site_biomass_test <- constants$x_by_site[test_sites, ]
		...
		fold_inits$lambda <- inits$lambda[-test_counties]
		...
		fold_inits$biomass_site <- mc_extract(chains, 'biomass_site', j = TRUE)[-test_sites]
		...
		fold_monitors <- c('beta_occs', 'U', 'beta_biomass', 'biomass_site', 'sigma_biomass_within_sites')
