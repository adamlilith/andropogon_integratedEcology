...
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_05b_model_biomass_fx_of_environment_lognormal_heteroscedastic_preliminary.r')
### 
### CONTENTS ###
### setup ###
### model ###
...
###########################
	...
	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/biomass_alone_lognormal_heteroscedastic_bio12', ifelse(trial, '_TRIAL', ''), '/')

	# # gamma and lognormal
	# formula_biomass <- ~ 1 + bio12 + I(bio12^2)
	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/biomass_alone_lognormal_heteroscedastic_bio12^2', ifelse(trial, '_TRIAL', ''), '/')

	# # lognormal
	# formula_biomass <- ~ 1 + bio1 + bio12 + I(bio1^2)
	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/biomass_alone_lognormal_heteroscedastic_bio1^2_bio12', ifelse(trial, '_TRIAL', ''), '/')

	# gamma & lognormal
	formula_biomass <- ~ 1 + bio1 + aridity + I(bio1^2)
	out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/biomass_alone_lognormal_heteroscedastic_bio1^2_aridity', ifelse(trial, '_TRIAL', ''), '/')

	# # gamma
	# formula_biomass <- ~ 1 + bio12 + aridity
	# out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/biomass_alone_lognormal_heteroscedastic_bio12_aridity', ifelse(trial, '_TRIAL', ''), '/')
...
#####################
	...
	inits <- list(
		...
		beta_mu_biomass = inits_beta_biomass, # PDM coefficients
		beta_sigma_biomass = rep(1, n_terms_biomass), # PDM coefficients
		
		site_mu_hat_biomass = rep(mean(biomass_data_raw$Biomass), n_pheno_sites), # latent mean biomass at a site
	...
	if (n_predictors_biomass > 1) {
		...
		model_code <- nimbleCode({
			...
			beta_mu_biomass[1] ~ dnorm(0, sd = 10) # intercept... not regularized
			beta_sigma_biomass[1] ~ dnorm(0, sd = 10) # intercept... not regularized
			for (i in 2:n_terms_biomass) {
				# beta_mu_biomass[i] ~ ddexp(0, rate = 1) # regularization toward 0
				beta_mu_biomass[i] ~ dnorm(0, sd = 10) # broad prior
				beta_sigma_biomass[i] ~ dnorm(0, sd = 10) # broad prior
			}

			# mean of biomass at a site is latent and functions of environment
			# plant biomasses are samples from the site-level distribution defined by the site's mean and global sd
			for (i in 1:n_pheno_sites) {

				site_mu_hat_biomass[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], x_by_site_biomass[i, 1:n_terms_biomass])
				log(site_sigma_hat_biomass[i]) <- inprod(beta_sigma_biomass[1:n_terms_biomass], x_by_site_biomass[i, 1:n_terms_biomass])

				# simulate mean site biomass for DHARMa residuals
				site_mu_hat_biomass_sim[i] ~ dlnorm(
					meanlog = site_mu_hat_biomass[i],
					taulog = 1 / site_sigma_hat_biomass[i]
				)
			...
			for (i in 1:n_biomass) {
				...
				# likelihood
				y_biomass[i] ~ dlnorm(
					meanlog = site_mu_hat_biomass[site_index_biomass[i]],
					taulog = 1 / site_sigma_hat_biomass[site_index_biomass[i]]
				)

				# simulated values for unconditional DHARMa residuals
				y_biomass_sim[i] ~ dlnorm(
					meanlog = site_mu_hat_biomass[site_index_biomass[i]],
					taulog = 1 / site_sigma_hat_biomass[site_index_biomass[i]]
				)
			...
			for (i in 1:n_counties) {
				...
				# biomass: status quo
				mu_biomass_county_sq[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], pheno_x_counties_sq[i, 1:n_terms_biomass])

				# biomass: future
				biomass_county_ssp245_2041_2070[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], pheno_x_counties_ssp245_2041_2070[i, 1:n_terms_biomass])
				
				biomass_county_ssp245_2071_2100[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], pheno_x_counties_ssp245_2071_2100[i, 1:n_terms_biomass])
				
				biomass_county_ssp370_2041_2070[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], pheno_x_counties_ssp370_2041_2070[i, 1:n_terms_biomass])

				biomass_county_ssp370_2071_2100[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], pheno_x_counties_ssp370_2071_2100[i, 1:n_terms_biomass])
			...
			for (j in 1:n_predictors_biomass) {
				...
				for (i in 1:n_response_curve_values) {
					...
					pheno_response_curves_biomass[i, j] <-
						inprod(beta_mu_biomass[1:n_terms_biomass], response_curve_x_biomass[i, 1:n_terms_biomass, j])
		...
		### model where formulae for mean biomass has >1 predictor: reduced model WITHOUT posterior predictive nodes
		############################################################################################################
		...
		model_code <- nimbleCode({
			...
			for (i in 1:n_pheno_sites) {
				...
				site_mu_hat_biomass[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], x_by_site_biomass[i, 1:n_terms_biomass])
				log(site_sigma_hat_biomass[i]) <- inprod(beta_mu_biomass[1:n_terms_biomass], x_by_site_biomass[i, 1:n_terms_biomass])
			...
			for (i in 1:n_response_curve_values) {
				...
				pheno_response_curves_biomass[i] <-
					inprod(beta_mu_biomass[1:n_terms_biomass], response_curve_x_biomass[i, 1:n_terms_biomass])
				
			}

		})

		### model where formulae for mean biomass has just 1 predictor: reduced model WITHOUT posterior predictive nodes
		################################################################################################################
		model_code_simple <- nimbleCode({
	...
	monitors_coeffs <- c('beta_mu_biomass', 'beta_sigma_biomass', 'site_mu_hat_biomass', 'site_sigma_hat_biomass')
	monitors_simple <- c('beta_mu_biomass', 'beta_sigma_biomass')
...
say('#########################')
	...
	mcmc <- chains$samples
	cols <- c(
		paste0('beta_mu_biomass[', 1:n_terms_biomass, ']'),
		paste0('beta_sigma_biomass[', 1:n_terms_biomass, ']'),
		paste0('site_mu_hat_biomass[', 1:n_pheno_sites, ']'),
		paste0('site_sigma_hat_biomass[', 1:n_pheno_sites, ']')
	)
	for (i in 1:nchains) {
		mcmc[[i]] <- mcmc[[i]][ , cols]
	}
	...
	file <- paste0(out_dir, '/beta_biomass_sigma_sample_correlations.png')
	ggsave(ggs_crosscorrelation(ggs_mcmc, family = pars), file = file, width = 8, height = 8, dpi = 450, bg = 'white')

	# graphing trace and density plots for all site-level biomass
	pars <- paste0('site_mu_hat_biomass[', 1:n_pheno_sites, ']')
	...
	ggsave(mcmc_dens_chains(mcmc, pars = pars), file = file, width = 10, height = 8, dpi = 450, bg = 'white')

	# graphing trace and density plots for all site-level biomass
	pars <- paste0('site_sigma_hat_biomass[', 1:n_pheno_sites, ']')
	...
	sims_by_site <- hammer_subset(chains, param = 'site_mu_hat_biomass_sim', j = TRUE)
	sims_by_site <- hammer_rbind(sims_by_site)
	...
	for (i in seq_along(linear_terms)) {
		...
		this_x <- data.frame(
			x = site_data_raw[[linear_terms[i]]],
			y = resids
		)
...
say('########################')
	...
	mean_biomass_by_site <- biomass_data_raw[ , .(mean_biomass = mean(Biomass, na.rm = TRUE)), by = SITE]
	stand_resid <- (mean_biomass_by_site$mean_biomass - exp(resp$all.chains[ , 'Mean'])) / mean_biomass_by_site$mean_biomass
...
say('##############################')
	...
	ag_vect_sq$resp_mean <- exp(resp[ , 'Mean']) + 0.001 # add a small value to avoid log(0)
	ag_vect_sq$resp_0.05ci <- exp(resp[ , '95%CI_low'])
	ag_vect_sq$resp_0.95ci <- exp(resp[ , '95%CI_upp'])
	ag_vect_sq$resp_ci <- exp(ag_vect_sq$resp_0.95ci) - exp(ag_vect_sq$resp_0.05ci)

	# # an island in NW Canada has unrealistically high biomass predictions bc extremely wet
	# if (any(ag_vect_sq$resp_mean > 1e5)) {
	# 	ag_vect_sq <- ag_vect_sq[ag_vect_sq$resp_mean < 1e5]
	# }
	...
	# resp_limits <- c(0, max(c(ag_vect_sq_pres$resp_mean, site_data_with_biomass_vect$Biomass)))
...
say('##############################')
	...
	for (fut in futs) {
		...
		# get just counties with data
		this_ag_vect$resp_mean <- exp(resp[ , 'Mean']) + 0.001 # add a small value to avoid log(0)
		this_ag_vect$resp_0.05ci <- exp(resp[ , '95%CI_low'])
		this_ag_vect$resp_0.95ci <- exp(resp[ , '95%CI_upp'])
		this_ag_vect$resp_ci <- exp(this_ag_vect$resp_0.95ci) - exp(this_ag_vect$resp_0.05ci)
...
say('########################################')
	...
	which_resp <- grepl(rownames(summary), pattern = paste0('mu_biomass_county_sq'))
	resp <- summary[which_resp, 'Mean']
	resp <- exp(resp) + 0.001 # add a small value to avoid log(0)
