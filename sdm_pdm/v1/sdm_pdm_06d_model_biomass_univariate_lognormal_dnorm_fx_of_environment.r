...
###
### This model estimates the distribution of site-level mean Andropogon gerardi ramet biomass. The distribution of biomasses among ramets at a site follows a gamma distribution with individual ramet biomasses drawn from this distribution. The site-level mean biomass is a draw from a lognormal distribution, the mean of which is a a draw from another normal. The mean of this second normal is a function of soil/climate, and assumed to respond to one climatic predictor (possibly with higher-order terms).
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_06d_model_biomass_univariate_lognormal_dnorm_fx_of_environment.r')
...
###########################
	...
	out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_biomass_fx_of_environment/biomass_lognormal_dnorm_fx_of_environment_sand', ifelse(trial, '_TRIAL', ''), '/')
	...
	formula_biomass_mu <- ~ 1 + sand # response of biomass to environment
...
#############
	...
	say('This model estimates the distribution of site-level mean Andropogon gerardi ramet biomass. The distribution of biomasses among ramets at a site follows a gamma distribution with individual ramet biomasses drawn from this distribution. The site-level mean biomass is a draw from a lognormal distribution, the mean of which is a a draw from another normal. The mean of this second normal is a function of soil/climate, and assumed to respond to one climatic predictor (possibly with higher-order terms).', breaks = 60, post = 1)
...
######################################################
	...
	descrip <- paste0(trait, ': lognorma~dnorm homoscedastic')
