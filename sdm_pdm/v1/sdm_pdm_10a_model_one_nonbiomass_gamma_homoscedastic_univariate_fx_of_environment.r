...
###
### This model estimates the distribution of a SINGLE non-biomass trait's site-level mean value. The distribution of trait values among ramets at a site is assumed to follow a gamma distribution with individual ramet values drawn from this distribution. The site-level mean is a function of environmental covariates. The model is "explicitly" homoscedastic because the standard deviation is not assumed to vary by site. The mean of the gamma is a draw from the a normal distribution with a mean given by the function of the environment. The function allows for ONE climatic and/or edaphic predictor (possibly with higher-order terms).
###
...
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_10a_model_one_nonbiomass_gamma_homoscedastic_univariate_fx_of_environment.r')
...
###########################
	...
	for (trait in traits) {
		...
		for (count_form in seq_along(formulae_traits)) {
			...
			say('This model estimates the distribution of a SINGLE non-biomass trait\'s site-level mean value. The distribution of trait values among ramets at a site is assumed to follow a gamma distribution with individual ramet values drawn from this distribution. The site-level mean is a function of environmental covariates. The model is "explicitly" homoscedastic because the standard deviation is not assumed to vary by site. The mean of the gamma is a draw from the a normal distribution with a mean given by the function of the environment. The function allows for ONE climatic and/or edaphic predictor (possibly with higher-order terms).', breaks = 60, post = 1)
			...
			model_code <- nimbleCode({
				...
				for (i in 1:n_trait_values) {
					...
					y_traits[i, 1] ~ dgamma(shape = shape_traits[site_index_traits[i]], rate = rate_traits[site_index_traits[i]])

					# simulated values for unconditional DHARMa residuals
					y_traits_sim[i] ~ dgamma(shape = shape_traits[site_index_traits[i]], rate = rate_traits[site_index_traits[i]])
			
					log_lik_y_traits[i] <- dgamma(y_traits[i, 1], shape = shape_traits[site_index_traits[i]], rate = rate_traits[site_index_traits[i]], log = 1)
