...
###
### This model estimates the distribution of a SINGLE non-biomass trait's site-level mean value. The distribution of trait values among ramets at a site is assumed to follow a zero-inflated gamma distribution with individual ramet values drawn from this distribution. The site-level mean, standard deviation, and the probability of a zero value are each functions of environmental covariates, and have the same functional forms (but not coefficients). The mean of the gamma is a draw from the a normal distribution with a mean given by the function of the environment. The model is "explicitly" heteroscedastic because the standard deviation is not assumed to be constant. The function allows for ONE climatic and/or edaphic predictor (possibly with higher-order terms).
###
...
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_10c_model_one_nonbiomass_gamma_heteroscedastic_univariate_fx_of_environment.r')
...
###########################
	...
	for (trait in traits) {
		...
		for (count_form in seq_along(formulae_traits)) {
			...
			out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_', trait, '/', trait, '_gamma~normal_heteroscedastic_', terms, ifelse(trial, '_TRIAL', ''), '/')
			...
			say('This model estimates the distribution of a SINGLE non-biomass trait\'s site-level mean value. The distribution of trait values among ramets at a site is assumed to follow a zero-inflated gamma distribution with individual ramet values drawn from this distribution. The site-level mean, standard deviation, and the probability of a zero value are each functions of environmental covariates, and have the same functional forms (but not coefficients). The mean of the gamma is a draw from the a normal distribution with a mean given by the function of the environment. The model is "explicitly" heteroscedastic because the standard deviation is not assumed to be constant. The function allows for ONE climatic and/or edaphic predictor (possibly with higher-order terms).', breaks = 60, post = 1)
			...
			inits <- list(
				...
				beta_traits_sigma = rep(0, data_traits$n_terms_traits),
			...
			model_code <- nimbleCode({
				...
				# NON-BIOMASS TRAITS
				# priors for relationship of mean to environment
				for (i in 1:n_terms_traits) {
					beta_traits_mu[i] ~ dnorm(0, sd = 10) # broad prior
				}

				# priors for spread of site-level trait values
				for (i in 1:n_terms_traits) {
					beta_traits_sigma[i] ~ dnorm(0, sd = 10) # broad prior
				}

				# priors for probability of trait having zero value
				for (i in 1:n_terms_traits) {
				...
				for (i in 1:n_pheno_sites) {
					...
					#sd of site-level trait values
					cross_site_traits_sigma_log[i] <- inprod(beta_traits_sigma[1:n_terms_traits], x_by_site_traits[i, 1:n_terms_traits])
					cross_site_traits_sigma[i] <- exp(cross_site_traits_sigma_log[i])

					# relationship of trait to the environment
					...
					# site-level mean trait
					site_traits_mu[i] <- exp(log_site_traits_mu[i])

					# moment matching to get dzigamma() parameters
					shape_traits[i] <- site_traits_mu[i]^2 / cross_site_traits_sigma[i]^2
					rate_traits[i] <- site_traits_mu[i] / cross_site_traits_sigma[i]^2
				...
				for (i in 1:n_counties) {
					...
					cross_site_traits_sigma_log_county_sq[i] <- inprod(beta_traits_sigma[1:n_terms_traits], counties_x_traits_sq[i, 1:n_terms_traits])
					cross_site_traits_sigma_county_sq[i] <- exp(cross_site_traits_sigma_log_county_sq[i])

					logit(pzero_traits_county_sq[i]) <- inprod(beta_traits_pzero[1:n_terms_traits], counties_x_traits_sq[i, 1:n_terms_traits])

					shape_traits_county_mu_sq[i] <- exp_traits_mu_county_sq[i]^2 / cross_site_traits_sigma_county_sq[i]^2
					rate_traits_county_mu_sq[i] <- exp_traits_mu_county_sq[i] / cross_site_traits_sigma_county_sq[i]^2
					...
					# non-trait traits: future ssp245_2041_2070
					...
					cross_site_traits_sigma_log_county_ssp245_2041_2070[i] <- inprod(beta_traits_sigma[1:n_terms_traits], counties_x_traits_ssp245_2041_2070[i, 1:n_terms_traits])
					cross_site_traits_sigma_county_ssp245_2041_2070[i] <- exp(cross_site_traits_sigma_log_county_ssp245_2041_2070[i])

					shape_traits_county_mu_ssp245_2041_2070[i] <- exp_traits_mu_county_ssp245_2041_2070[i]^2 / cross_site_traits_sigma_county_ssp245_2041_2070[i]^2
					rate_traits_county_mu_ssp245_2041_2070[i] <- exp_traits_mu_county_ssp245_2041_2070[i] / cross_site_traits_sigma_county_ssp245_2041_2070[i]^2
					...
					# non-trait traits: future ssp245_2071_2100
					...
					cross_site_traits_sigma_log_county_ssp245_2071_2100[i] <- inprod(beta_traits_sigma[1:n_terms_traits], counties_x_traits_ssp245_2071_2100[i, 1:n_terms_traits])
					cross_site_traits_sigma_county_ssp245_2071_2100[i] <- exp(cross_site_traits_sigma_log_county_ssp245_2071_2100[i])
					...
					shape_traits_county_mu_ssp245_2071_2100[i] <- exp_traits_mu_county_ssp245_2071_2100[i]^2 / cross_site_traits_sigma_county_ssp245_2071_2100[i]^2
					rate_traits_county_mu_ssp245_2071_2100[i] <- exp_traits_mu_county_ssp245_2071_2100[i] / cross_site_traits_sigma_county_ssp245_2071_2100[i]^2
					...
					# non-trait traits: future ssp370_2041_2070
					...
					cross_site_traits_sigma_log_county_ssp370_2041_2070[i] <- inprod(beta_traits_sigma[1:n_terms_traits], counties_x_traits_ssp370_2041_2070[i, 1:n_terms_traits])
					cross_site_traits_sigma_county_ssp370_2041_2070[i] <- exp(cross_site_traits_sigma_log_county_ssp370_2041_2070[i])

					shape_traits_county_mu_ssp370_2041_2070[i] <- exp_traits_mu_county_ssp370_2041_2070[i]^2 / cross_site_traits_sigma_county_ssp370_2041_2070[i]^2
					rate_traits_county_mu_ssp370_2041_2070[i] <- exp_traits_mu_county_ssp370_2041_2070[i] / cross_site_traits_sigma_county_ssp370_2041_2070[i]^2
					...
					# non-trait traits: future ssp370_2071_2100
					...
					cross_site_traits_sigma_log_county_ssp370_2071_2100[i] <- inprod(beta_traits_sigma[1:n_terms_traits], counties_x_traits_ssp370_2071_2100[i, 1:n_terms_traits])
					cross_site_traits_sigma_county_ssp370_2071_2100[i] <- exp(cross_site_traits_sigma_log_county_ssp370_2071_2100[i])
					...
					shape_traits_county_mu_ssp370_2071_2100[i] <- exp_traits_mu_county_ssp370_2071_2100[i]^2 / cross_site_traits_sigma_county_ssp370_2071_2100[i]^2
					rate_traits_county_mu_ssp370_2071_2100[i] <- exp_traits_mu_county_ssp370_2071_2100[i] / cross_site_traits_sigma_county_ssp370_2071_2100[i]^2
				...
				for (j in 1:n_covariates_traits) {
					for (i in 1:n_response_curve_values) {
						...
						log_response_curves_traits_sigma[i, j] <-
							inprod(beta_traits_sigma[1:n_terms_traits], resp_curves_x_traits[i, 1:n_terms_traits, j])
						
						response_curves_traits_sigma[i, j] <- exp(log_response_curves_traits_sigma[i, j])
			...
			monitors_coeffs_single_index <- c(
				'beta_traits_mu', 'beta_traits_sigma', 'beta_traits_pzero'
			)
			...
			monitors_derived_single_index <- c(
				'site_traits_mu', 'cross_site_traits_sigma'
			)
			...
			monitors_resp_curves <- c(
				'response_curves_traits_mu', 'response_curves_traits_sigma'
			)
			...
			vars <- c('beta_traits_mu', 'beta_traits_sigma', 'beta_traits_pzero')
			...
			homoscedastic <- FALSE
			descrip <- paste0(trait, ': zigamma heteroscedastic')
