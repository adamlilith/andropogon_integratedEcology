...
### The biomass component is based on field observations at 26 sites at which the aboveground biomass of several plants were measured. The model assumes that the distribution of biomass *at a site* is drawn from a gamma distribution, where the mean is a function of environmental covariates. Each ramet's biomass is a draw from the site-level gamma distribution.
### The integration assumes that the "normal" component of the lognormal distribution of the SDM and "normal" representation of the mean of biomass are drawn from a multivariate normal distribution. Off-diagonal elements of the VCV matrix are estimated.
###
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_09_model_[integration_mvnorm]_[occurrence]_[biomass_univariate_gamma].r')
###
#############
### setup ###
#############
...
###########################
	...
	out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_integrated_occurrence_biomass/[occs_bio1^2_bio12^2_bio15^2_[bias_quad_ia]]_[biomass_gamma_bio12]', ifelse(trial, '_TRIAL', ''), '_set_4/')
	...
	formula_biomass_mu <- ~ 1 + bio12 # response ot AG biomass to environment
	...
	# ### MCMC settings
	# need these settings to achieve independent samples using bio1^2, bio12^2, bio15^2
	niter <- 240000 * 8
	nburnin <- 40000 * 8
	thin <- 200 * 8
	nchains <- 1
...
#############
	...
	say('The biomass component is based on field observations at 26 sites at which the aboveground biomass of several plants were measured. The model assumes that the distribution of biomass *at a site* is drawn from a gamma distribution, where the mean is a function of environmental covariates. Each ramet\'s biomass is a draw from the site-level gamma distribution.', breaks = 80)
	say('The integration assumes that the "normal" component of the lognormal distribution of the SDM and "normal" representation of the mean of biomass are drawn from a multivariate normal distribution. Off-diagonal elements of the VCV matrix are estimated.', breaks = 80)
	...
	observed_site_agg <- data_biomass_mu$raw_data_biomass[ , .(mean_biomass = mean(Biomass)), by = SITE]
	observed_site_mean_biomass <- observed_site_agg$mean_biomass

	# initialization for integration
	Phi_inits <- matrix(c(rep(2, data_occs$n_counties), rep(log(mean(data_biomass_mu$y_biomass)), data_biomass_mu$n_counties)), byrow = FALSE, ncol = 2)

	# Calculate standard deviation of biomass across sites
	sd_site_biomass <- data_biomass_mu$raw_data_biomass[ , .(sd_biomass = sd(Biomass)), by = SITE][['sd_biomass']]
	site_biomass_plant_sigma_log <- log(mean(sd_site_biomass))
	...
	inits <- list(
		...
		log_site_biomass_mu = log(observed_site_mean_biomass), # mean site-level biomass

		site_biomass_plant_sigma_log = log(sd(data_biomass_mu$y_biomass)),

		beta_biomass_mu = beta_biomass_mu_inits,
		# beta_biomass_mu_reg_log = 1, # regularization penalty for biomass betas
	...
	model_code <- nimbleCode({
		...
		for (i in 1:n_counties) {
			...
			N_ag_county_sq[i] ~ dpois(lambda_mu_sq[i]) # posterior predictive sampler
			...
			mu_biomass_county_sq[i] <- exp(Phi[i, 2])
	...
	### OCCURRENCE PRIORS & RESPONSE CURVES
	#######################################
		...
		beta_occs_mu[1] ~ dnorm(0, sd = 100)
		for (i in 2:n_terms_occs) {
			# beta_occs_mu[i] ~ dnorm(0, sd = 20)
			beta_occs_mu[i] ~ ddexp(0, rate = 0.7675)
		}

		# OCCURRENCE: priors for sampling bias
		# alpha_occs[1] ~ dnorm(0, sd = 10)
		...
		# We're assuming occurrence responds to just two or more environmental predictors, so the response curve "x" is an array with one "page" per predictor and output is a matrix with one column per predictor
	...
	### BIOMASS LIKELIHOOD AND RESPONSE CURVES
	##########################################

		# BIOMASS: priors for relationship of mean and variance to environment
		# ddexp(): rate = 0.7675 sets |beta| < 3 90% of time and <5 97.8% of the time, and <10 99.95% of the time
		# ddexp(): rate = 0.9986 sets |beta| < 3 95% of time
		beta_biomass_mu[1] ~ dnorm(0, sd = 100)
		for (i in 2:n_terms_biomass_mu) {
			# beta_biomass_mu[i] ~ dnorm(0, sd = 100) # broad prior
			beta_biomass_mu[i] ~ ddexp(0, rate = 0.7675)
		}

		# site_biomass_plant_sigma_log ~ dhalfflat()
		# site_biomass_plant_sigma_log ~ dunif(0, 100)

		site_biomass_plant_sigma_log ~ dnorm(0, sd = 2.5) # broad half-Cauchy
		...
		for (i in 1:n_pheno_sites) {
			...
			log_site_biomass_mu[i] ~ dnorm(phi_site_biomass_mu[i], sd = site_biomass_plant_sigma_log)
		...
		for (i in 1:n_biomass) {
			...
			# likelihood
			log_lik_biomass_y[i] <- dgamma(y_biomass[i], shape = shape_biomass[site_index_biomass[i]], rate = rate_biomass[site_index_biomass[i]], log = 1)
	...
	#######################################################################
		...
		for (i in 1:n_counties) {
			...
			mu_biomass_county_ssp245_2041_2070[i] <- exp(Phi_county_ssp245_2041_2070[i, 2])
			...
			mu_biomass_county_ssp245_2071_2100[i] <- exp(Phi_county_ssp245_2071_2100[i, 2])
			...
			mu_biomass_county_ssp370_2041_2070[i] <- exp(Phi_county_ssp370_2041_2070[i, 2])
			...
			mu_biomass_county_ssp370_2071_2100[i] <- exp(Phi_county_ssp370_2071_2100[i, 2])
	...
	# conf$addSampler(target = c('beta_biomass_mu', 'alpha_occs', 'beta_occs_mu'), type = 'AF_slice')
	# say('AF_slice sampler added to beta_biomass_mu, beta_occs_mu, and alpha_occs.')

	# AF_slice samplers for correlated parameters
	conf$removeSamplers('beta_biomass_mu')
	conf$addSampler(target = 'beta_biomass_mu', type = 'AF_slice')
	say('AF_slice sampler added to beta_biomass_mu')

	conf$removeSamplers('alpha_occs')
	conf$removeSamplers('beta_occs_mu')
	conf$addSampler(target = c('alpha_occs', 'beta_occs_mu'), type = 'AF_slice')
	say('AF_slice sampler added to beta_occs_mu and and alpha_occs.')
...
say('#########################')
	...
	workflow_postmodeling_generic(facet = 'occurrence & biomass', formulae = formulae, descrip = 'occurrence: Poisson lognormal homoscedastic; biomass: gamma homoscedastic', out_dir = out_dir)
	workflow_postmodeling_occurrence(homoscedastic = TRUE, pred_vect_nam = pred_vect_nam, out_dir = out_dir)
	workflow_postmodeling_biomass(homoscedastic = TRUE, pred_vect_nam = pred_vect_nam, out_dir = out_dir)
