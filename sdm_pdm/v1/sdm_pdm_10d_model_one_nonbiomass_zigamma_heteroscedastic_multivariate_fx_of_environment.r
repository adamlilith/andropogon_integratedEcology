...
###
### This model estimates the distribution of a SINGLE non-biomass trait's site-level mean value. The distribution of trait values among ramets at a site is assumed to follow a zero-inflated gamma distribution with individual ramet values drawn from this distribution. The site-level mean, standard deviation, and the probability of a zero value are each functions of environmental covariates, and have the same functional forms (but not coefficients). The model is "explicitly" heteroscedastic because the standard deviation is not assumed to be constant. The mean of the gamma is a draw from the a normal distribution with a mean given by the function of the environment.  The function allows for MORE THAN ONE climatic and/or edaphic predictor (WITHOUT higher-order terms).
###
...
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_10d_model_one_nonbiomass_gamma_heteroscedastic_multivariate_fx_of_environment.r')
