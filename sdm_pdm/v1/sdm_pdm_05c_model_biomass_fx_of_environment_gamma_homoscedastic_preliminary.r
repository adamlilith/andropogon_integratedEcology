...
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_05c_model_biomass_fx_of_environment_gamma_homoscedastic_preliminary.r')
### 
### CONTENTS ###
### setup ###
### model ###
...
#####################
	...
	inits <- list(
		...
		shape_biomass = 1 / init_biomass_sigma, # shape parameter for gamma distribution of biomass
	...
	if (n_predictors_biomass > 1) {
		...
		model_code <- nimbleCode({
			...
			shape_biomass ~ dgamma(0.001, 0.001)
			...
			for (i in 1:n_pheno_sites) {
				...
				log_site_mu_hat_biomass[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], x_by_site_biomass[i, 1:n_terms_biomass])
				site_mu_hat_biomass[i] <- exp(log_site_mu_hat_biomass[i])
				rate_biomass[i] <- shape_biomass / site_mu_hat_biomass[i]
			...
			for (i in 1:n_biomass) {
				...
				# likelihood
				y_biomass[i] ~ dgamma(shape = shape_biomass, rate = rate_biomass[site_index_biomass[i]])

				# simulated values for unconditional DHARMa residuals
				y_biomass_sim[i] ~ dgamma(shape_biomass, rate_biomass[site_index_biomass[i]])
			...
			for (i in 1:n_counties) {
				...
				# biomass: status quo
				log_biomass_county_mu_sq[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], pheno_x_counties_sq[i, 1:n_terms_biomass])
				mu_biomass_county_sq[i] <- exp(log_biomass_county_mu_sq[i])

				# biomass: future
				log_biomass_county_ssp245_2041_2070[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], pheno_x_counties_ssp245_2041_2070[i, 1:n_terms_biomass])
				biomass_county_ssp245_2041_2070[i] <- exp(log_biomass_county_ssp245_2041_2070[i])
				
				log_biomass_county_ssp245_2071_2100[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], pheno_x_counties_ssp245_2071_2100[i, 1:n_terms_biomass])
				biomass_county_ssp245_2071_2100[i] <- exp(log_biomass_county_ssp245_2071_2100[i])
				
				log_biomass_county_ssp370_2041_2070[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], pheno_x_counties_ssp370_2041_2070[i, 1:n_terms_biomass])
				biomass_county_ssp370_2041_2070[i] <- exp(log_biomass_county_ssp370_2041_2070[i])

				log_biomass_county_ssp370_2071_2100[i] <- inprod(beta_mu_biomass[1:n_terms_biomass], pheno_x_counties_ssp370_2071_2100[i, 1:n_terms_biomass])
		...
		model_code <- nimbleCode({
			...
			for (i in 1:n_biomass) {
				...
				# likelihood
				y_biomass[i] ~ dgamma(shape_biomass, rate_biomass[site_index_biomass[i]])
	...
	monitors_simple <- c('beta_mu_biomass', 'shape_biomass')

	monitors_coeffs <- c('beta_mu_biomass', 'site_mu_hat_biomass', 'shape_biomass')
	monitors <- c(
		monitors_coeffs,
...
say('#########################')
	...
	# graphing trace and density plots for shape_biomass
	pars <- paste0('shape_biomass')
	file <- paste0(out_dir, '/biomass_shape_biomass_trace.png')
	ggsave(mcmc_trace(mcmc, pars = pars), file = file, width = 10, height = 8, dpi = 450, bg = 'white')

	file <- paste0(out_dir, '/biomass_shape_biomass_density.png')
	ggsave(mcmc_dens_chains(mcmc, pars = pars), file = file, width = 10, height = 8, dpi = 450, bg = 'white')
