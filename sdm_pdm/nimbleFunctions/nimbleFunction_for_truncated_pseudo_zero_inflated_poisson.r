#' Truncated, pseudo-zero-inflated Poisson distribution
#'
#' Distribution and random number generator function for a truncated, pseudo-zero inflated Poisson distribution. The pseudo-zero inflation is reflected in supplying the functions a value `z`, which corresponds to a zero value of the Poisson when zero and a non-zero value when one. The Poisson is truncated because zeros are only possible when `z` is zero (i.e., so exogenous to the distribution function).
#'
#' @param x Numeric vector.
#' @param p Numeric vector of probabilities.
#' @param q Numeric vector of quantiles.
#' @param z Numeric equal to 0 or 1, indicating absence or presence.
#' @param lambda Numeric >= 0. Expected density.
#' @param log Logical (if `TRUE` return the log value), or 0 (`FALSE`)/1 (`TRUE`).
#' @param n Number of values.
dTruncPseudoZIP <- nimbleFunction(
    run = function(
        x = double(),
        lambda = double(),
        z = double(),
