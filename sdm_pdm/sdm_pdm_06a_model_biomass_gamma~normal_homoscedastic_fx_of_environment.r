...
###
### source('C:/Kaji/R/andropogon_integratedEcology/sdm_pdm/sdm_pdm_06a_model_biomass_gamma~normal_homoscedastic_fx_of_environment.r')
...
###########################
	...
	out_dir <- paste0('./outputs_loretta/integrated_sdm_pdm/models_biomass/[biomass_gamma~normal_homoscedastic_bio12]', ifelse(trial, '_TRIAL', ''), '/')
...
#############
	...
	say('This model estimates the distribution of site-level mean Andropogon gerardi ramet biomass. The distribution of biomasses among ramets at a site follows a gamma distribution with individual ramet biomasses drawn from this distribution. The site-level mean biomass is a function of soil/climate. The mean is assumed to respond to one or more climatic predictors (possibly with higher-order terms).', breaks = 60, post = 1)
	...
	inits <- list(
		...
		sigma_biomass_within_sites_log = 1,
		...
		log_biomass_county_mu_fifties = rep(0, data_biomass_mu$n_counties_20th_cent)
	...
	model_code <- nimbleCode({
		...
		for (i in 1:n_counties) {
			...
			log_biomass_county_mu_sq[i] ~ dnorm(log_biomass_county_mu_unscaled_sq[i], sd = sigma_biomass_among_sites)
			...
			log_biomass_county_mu_ssp245_2041_2070[i] ~ dnorm(log_biomass_county_mu_unscaled_ssp245_2041_2070[i], sd = sigma_biomass_among_sites)
			...
			log_biomass_county_mu_ssp245_2071_2100[i] ~ dnorm(log_biomass_county_mu_unscaled_ssp245_2071_2100[i], sd = sigma_biomass_among_sites)
			...
			log_biomass_county_mu_ssp370_2041_2070[i] ~ dnorm(log_biomass_county_mu_unscaled_ssp370_2041_2070[i], sd = sigma_biomass_among_sites)
			...
			log_biomass_county_mu_ssp370_2071_2100[i] ~ dnorm(log_biomass_county_mu_unscaled_ssp370_2071_2100[i], sd = sigma_biomass_among_sites)
		...
		for (i in 1:n_counties_20th_cent) {
			...
			log_biomass_county_mu_thirties[i] ~ dnorm(log_biomass_county_mu_unscaled_thirties[i], sd = sigma_biomass_among_sites)
			...
			log_biomass_county_mu_fifties[i] ~ dnorm(log_biomass_county_mu_unscaled_fifties[i], sd = sigma_biomass_among_sites)
			log_biomass_county_mu_unscaled_fifties[i] <-
			...
			for (i in 1:n_covariates_biomass) {
				...
				for (j in 1:n_response_curve_values) {
					...
					log_response_curves_biomass_mu[j, i] <-
						inprod(beta_biomass_mu[1:n_terms_biomass_mu], resp_curves_x_biomass[j, 1:n_terms_biomass_mu])
	...
	monitors_coeffs_not_indexed <- c(
		'sigma_biomass_within_sites_log', 'sigma_biomass_among_sites'
	)
...
######################################################
	...
	workflow_postmodeling_biomass(descrip = descrip, formula_biomass_mu = formula_biomass_mu, homoscedastic = homoscedastic, pred_vect_nam = pred_vect_nam, pred_vect_conus = pred_vect_conus, out_dir = out_dir)
