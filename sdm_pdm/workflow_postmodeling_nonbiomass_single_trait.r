#' Post-modeling workflow for non-biomass models with ONE trait and models with a single non-biomass trait component where it does not depend on other traits or abundance. Assumes that the number of predictor variables is ONE (but possibly with higher-order terms).
#'
#' @param trait Name of trait.
#' @param formula_traits Traits formula.
#' @param homoscedastic If `TRUE`, then this is a homoscedastic model.
#' @param pred_vect_nam SpatVector with posterior predictions of biomass for each county.
#' @param out_dir Folder in which to save results.
workflow_postmodeling_nonbiomass_single_trait <- function(trait, formula_traits, homoscedastic, descrip, pred_vect_nam, out_dir = out_dir) {
	...
	dharma <- createDHARMa(simulatedResponse = sims_by_plant, observedResponse = data_traits$y_traits, fittedPredictedResponse = fits, integerResponse = FALSE)
	...
	for (i in 1:constants$n_pheno_sites) residuals_by_site[i] <- mean(residuals_by_plant[data_traits$raw_data_traits$SITE == site_ids[i]])
	...
	# # sink(paste0(out_dir, '/dharma_residuals_spatial_autocorrelation_', trait, '.txt'), split = TRUE)
	# # say(paste0('MORAN\'S I TEST FOR SPATIAL AUTOCORRELATION IN DHARMa RESIDUALS OF ', trait))
	# # say(date(), post = 2)
	# # print(moran)
	# # sink()
	...
	if (!homoscedastic) {
		...
		say('Correlation between site-level mean observed and estimated s.d. of taut values: ', correl_sigma)
	...
	### NON-BIOMASS TRAIT: current map
	##################################
	say('NON-BIOMASS TRAIT: current map', level = 2)
		...
		nice_trait <- get_nice_trait(trait)
		trait_title <- tolower(nice_trait$short)
		title <- bquote('Present-day Mean ' * italic('Andropogon gerardi') * ' ' * .(trait_title))
		subtitle <- paste0('1991-2020 | ', nice_trait$short, ' ', mean_form)

		map_mu_sq <- map_nonbiomass_trait(
			out_dir = out_dir,
			filename_append = 'present',
			pred_vect_nam = pred_vect_nam,
			trait = trait,
			response_var = paste0(trait, '_mu_county_sq'),
		...
		nice_trait <- get_nice_trait(trait)
		trait_title <- tolower(nice_trait$short)
		title <- bquote('Present-day Probability of Zero Value of ' * .(trait_title))
		subtitle <- paste0('1991-2020 | ', nice_trait$short, ' ', mean_form)

		map_pzero_sq <- map_pzero(
			out_dir = out_dir,
			filename_append = 'present',
			pred_vect_nam = pred_vect_nam,
			facet = trait,
			response_var = paste0(trait, '_pzero_county_sq'),
			data_traits = data_traits,
			title = title,
			subtitle = subtitle,
			plot_range_core = FALSE,
	...
	### NON-BIOMASS TRAIT: future maps
	##################################
	say('NON-BIOMASS TRAIT: future maps', level = 2)
		...
		nice_trait <- get_nice_trait(trait)
		trait_title <- nice_trait$short

		for (fut in futs) {

			say(fut)

			title <- bquote('Future Mean ' * italic('Andropogon gerardi') * ' ' * .(trait_title))
			subtitle <- paste0('SSP ', substr(fut, 4, 6), ' ', substr(fut, 8, 11), '-', substr(fut, 13, 16), ' | ', trait_title, ' ', mean_form)
			response_var <- paste0(trait, '_mu_county_', fut)

			map_mu_fut <- map_nonbiomass_trait(
				out_dir = out_dir,
				filename_append = fut,
				pred_vect_nam = pred_vect_nam,
				trait = trait,
			...
			title <- bquote('Future Probability of Zero Value of ' * .(trait_title))
			subtitle <- paste0('SSP ', substr(fut, 4, 6), ' ', substr(fut, 8, 11), '-', substr(fut, 13, 16), ' | ', trait_title, ' ', mean_form)
			response_var <- paste0(trait, '_pzero_county_', fut)

			map_pzero_fut <- map_pzero(
				out_dir = out_dir,
				filename_append = fut,
				pred_vect_nam = pred_vect_nam,
				facet = trait,
	...
	### NON-BIOMASS TRAIT: future change maps
	#########################################
	say('NON-BIOMASS TRAIT: future change maps', level = 2)
		...
		for (fut in futs) {
			...
			title <- bquote('Change in ' * italic('Andropogon gerardi') * ' ' * .(trait_title))
			subtitle <- paste0('SSP ', substr(fut, 4, 6), ' ', substr(fut, 8, 11), '-', substr(fut, 13, 16), ' | ', trait_title, ' ', mean_form)

			response_var <- paste0(trait, '_mu_county_', fut)
			response_var_sq <- paste0(trait, '_mu_county_sq')

			map_traits_mu_change <- map_nonbiomass_traits_change(
				out_dir = out_dir,
				filename_append = fut,
				fut = fut,
				pred_vect_nam = pred_vect_nam,
				trait = trait,
				response_var = response_var,
				response_var_sq = response_var_sq,
				data_traits = data_traits,
				title = title,
				subtitle = subtitle,
				plot_range_core = FALSE,
			...
			title <- bquote('Change in Probability of Zero Value of ' * .(trait_title))
			subtitle <- paste0('SSP ', substr(fut, 4, 6), ' ', substr(fut, 8, 11), '-', substr(fut, 13, 16), ' | ', trait_title, ' ', mean_form)

			response_var <- paste0(trait, '_pzero_county_', fut)
			response_var_sq <- paste0(trait, '_pzero_county_sq')

			map_traits_pzero_change <- map_pzero_change(
				out_dir = out_dir,
				filename_append = fut,
				fut = fut,
				pred_vect_nam = pred_vect_nam,
	...
	say('NON-BIOMASS TRAIT: cross validation', level = 2)
		...
		if (!trial) {
			...
			# # write.csv(crossvalidation, paste0(out_dir, '/crossvalidation_', trait, '.csv'), row.names = FALSE)
	...
	#############
		...
		if (!homoscedastic) {
			meta_traits$fit_sigma = c(correl_sigma = correl_sigma, rmse_sigma = rmse_sigma, mean_error_sigma = mean_error_sigma, mean_abs_error_sigma = mean_abs_error_sigma)
		}

		if (!trial) {
			meta_traits$crossvalidation <- crossvalidation
		} else {
			meta_traits$crossvalidation <- NA
		}

		saveRDS(meta_traits, paste0(out_dir, '/!meta_nonbiomass_trait.rds'))
		sink(paste0(out_dir, '/!meta_nonbiomass_trait.txt'), split = TRUE)
			print(meta_traits)
		sink()
