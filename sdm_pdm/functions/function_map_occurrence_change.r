...
#'
#' out_dir    		Folder in which to save image. Use NULL to skip saving file.
...
#' response_var 	Name of response variable: 'N_ag_sq', 'N_ag_ssp245_2041_2070', 'N_ag_ssp245_2071_2100', 'N_ag_ssp370_2041_2070', 'N_ag_ssp370_2071_2100'.
...
map_occurrence_change <- function(
	...
	vars <- c('N_ag_mean_ssp245_2041_2070', 'N_ag_mean_ssp245_2071_2100', 'N_ag_mean_ssp370_2041_2070', 'N_ag_mean_ssp370_2071_2100')

	min_val <- Inf
	max_val <- -Inf
	x_sq <- pred_vect_display$N_ag_mean_sq[pred_vect_display$focal_region]
	for (var in vars) {
	...
	this_pred_vect$delta <- log10(this_pred_vect[[response_var]] / this_pred_vect$N_ag_mean_sq)
	...
	map <- ggplot() +
		...
		layer_spatial(data_traits$site_vect, pch = 3, size = 3) +
		layer_spatial(range_core_sq, color = 'gray20', fill = NA, linewidth = 1) +
		layer_spatial(range_core_fut, color = 'orange2', fill = NA, linewidth = 1) +
	...
	if (!is.null(out_dir)) ggsave(plot = map, filename = filename, width = 12, height = 10, dpi = 200)
	invisible(map)
