...
#' @param pred_vect_1930s SpatVector with predictions for CONUS
#' pred_vect_nam
map_occurrence_change_1930s <- function(zero_inflated, pred_vect_nam, pred_vect_1930s) {
	...
	range_core <- delineate_range_core(pred_vect_conus_restricted, column = 'N_ag_county_1930s', ag_core_quant = ag_core_quant)

	# change
	focal_n <- unlist(pred_vect_1930s[['N_ag_county_mean_1930s']])
	delta_vect$delta_n <- focal_n / delta_vect$N_ag_county_mean_sq
	
	focal_pzero <- unlist(pred_vect_1930s[['pzero_occs_county_1930s']])
	...
	if (!zero_inflated) {
		...
		title <- paste0('B) Probability of zero abundance')
