...
map_biomass_nonbiomass_change <- function(
	...
	column <- paste0('mu_', facet, '_county_', response_var_type, '_', fut)
	range_core_fut <- delineate_range_core(pred_vect_display, column = column, ag_core_quant)

	column <- paste0('mu_', facet, '_county_', response_var_type, '_sq')
	...
	min_val <- Inf
	max_val <- -Inf
	x_sq <- pred_vect_display[[paste0('mu_', facet, '_county_', response_var_type, '_sq')]]
	x_sq <- unlist(x_sq)
	...
	pred_vect_display$delta <- (pred_vect_display[[paste0('mu_', facet, '_county_', response_var_type, '_', fut)]] - pred_vect_display[[paste0('mu_', facet, '_county_', response_var_type, '_sq')]]) / pred_vect_display[[paste0('mu_', facet, '_county_', response_var_type, '_sq')]]
	...
	ggsave(plot = map, filename = paste0(out_dir, '/map_change_', facet, '_', response_var_type, '_', filename_append, '.png'), width = 12, height = 10, dpi = 200)
	invisible(map)
