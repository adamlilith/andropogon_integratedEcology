...
#' resp_distrib 		Named vector of response distributions. For occurrence, this can be 'Poisson' or 'ZIP'. For biomass this can be 'gamma', 'HG' (zero-inflated gamma), 'LN', or 'HLN' (zero-inflated lognormal)
...
burn_occs_biomass_into_vector <- function(
	...
	data_biomass <- prepare_occurrence_data(formula_occs = formula_biomass, formula_bias = ~ 1, n_response_curve_values = n_response_curve_values, psa_quant = psa_quant, calib = calib)
	...
	if (demesne == 'nam') {
		names(pred_vect)[index] <- c('N_ag_county_mean_sq', 'N_ag_county_median_sq', 'N_ag_county_inner_quant_sq', 'biomass_county_mean_sq', 'biomass_county_median_sq', 'biomass_county_inner_quant_sq')
	} else if (demesne == '1930s') {
		names(pred_vect)[index] <- c('N_ag_county_mean_1930s', 'N_ag_county_median_1930s', 'N_ag_county_inner_quant_1930s', 'biomass_county_mean_1930s', 'biomass_county_median_1930s', 'biomass_county_inner_quant_1930s')
	}
	...
	if (demesne == 'nam') {
		...
		for (fut in futs) {
			...
			x_biomass <- x_biomass[[1]]
			 ...
			 if (!zero_inflated) {
				...
				x_psi <- data_psi[paste0('counties_x_occs_', fut)]
				x_psi <- x_psi[[1]]
			}
			...
			index <- (ncol(pred_vect) - 5):ncol(pred_vect)
			names(pred_vect)[index] <- paste0(c('N_ag_county_mean_', 'N_ag_county_median_', 'N_ag_county_inner_quant_', 'biomass_county_mean_', 'biomass_county_median_', 'biomass_county_inner_quant_'), fut)
		...
		if (zero_inflated) {
			...
			for (fut in futs) {
				...
				x <- data_psi[paste0('counties_x_occs_', fut)]
				x <- x[[1]]
