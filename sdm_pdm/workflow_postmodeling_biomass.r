...
#' @param pred_vect_nam SpatVector with posterior predictions of biomass for each county in North America.
#' @param pred_vect_conus SpatVector with posterior predictions of biomass for each county in CONUS.
#' @param out_dir Folder in which to save results.
workflow_postmodeling_biomass <- function(descrip, formula_biomass_mu, homoscedastic, zero_inflated, pred_vect_nam, pred_vect_conus, out_dir) {
	...
	if (!homoscedastic) {
		...
		for (i in seq_len(data_biomass_sigma$n_covariates)) {

			if (data_biomass_sigma$covariates_biomass[i] == 'ph') {
				x <- data_biomass_sigma$raw_data_biomass[['site_soil_pH']]
			} else {
				x <- data_biomass_sigma$raw_data_biomass[[data_biomass_sigma$covariates_biomass[i]]]
			}
	...
	if (!homoscedastic) {
		...
		observed <- data_biomass_sigma$raw_data_biomass[ , .(sd_biomass = sd(Biomass)), by = SITE][['sd_biomass']]
		estimated <- hammer_extract(chains, 'site_biomass_sigma', j = TRUE)
	...
	say('BIOMASS: future maps', level = 2)
		...
		for (fut in futs) {
			...
			response_var <- paste0('biomass_mu_county_', fut)
	...
	map_thirties <- map_biomass_traits_change_1930s(facet = 'biomass', data_biomass_traits = data_biomass_mu, pred_vect_nam = pred_vect_nam, pred_vect_conus = pred_vect_conus)
	...
	for (k in 1:max_k_folds) {
		...
		fold_constants$site_index_biomass <- fold_constants$site_index_biomass[-indices]
		...
		crossvalidation <- rbind(
			...
			data.table(
				...
				correl_lower = quantile(correl, 0.05),
				correl_mean = mean(correl),
				correl_upper = quantile(correl, 0.95),

				mae_lower = quantile(mae, 0.05),
				mae_mean = mean(mae),
				mae_upper = quantile(mae, 0.95),

				mape_lower = quantile(mape, 0.05),
				mape_mean = mean(mape),
				mape_upper = quantile(mape, 0.95),

				rmse_lower = quantile(rmse, 0.05),
				rmse_mean = mean(rmse),
				rmse_upper = quantile(rmse, 0.95)
	...
	crossvalidation <- rbind(
		...
		data.table(
				...
				n_train_sites = mean(crossvalidation$n_train_sites),
				n_test_sites = mean(crossvalidation$n_test_sites),

				n_train_plants = mean(crossvalidation$n_train_plants),
				n_test_plants = mean(crossvalidation$n_test_plants),
				
				correl_lower = mean(crossvalidation$correl_lower),
				correl_mean = mean(crossvalidation$correl_mean),
				correl_upper = mean(crossvalidation$correl_upper),

				mae_lower = mean(crossvalidation$mae_lower),
				mae_mean = mean(crossvalidation$mae_mean),
				mae_upper = mean(crossvalidation$mae_upper),

				mape_lower = mean(crossvalidation$mape_lower),
				mape_mean = mean(crossvalidation$mape_mean),
				mape_upper = mean(crossvalidation$mape_upper),

				rmse_lower = mean(crossvalidation$rmse_lower),
				rmse_mean = mean(crossvalidation$rmse_mean),
				rmse_upper = mean(crossvalidation$rmse_upper)
		)
	)
	...
	###########
		...
		meta_biomass$crossvalidation <- crossvalidation
