...
map_biomass <- function(
	...
	if (response_var_type == 'mu') {
		min_val <- 0.95 * min(biomass_stats$biomass_mean)
		max_val <- 1.05 * max(biomass_stats$biomass_mean)
	} else {
		min_val <- 0.95 * min(biomass_stats$biomass_sd)
	...
	ggsave(plot = map, filename = paste0(out_dir, '/map_biomass_', response_var_type, '_', filename_append, '.png'), width = 12, height = 10, dpi = 600)
	invisible(map)
