...
map_occurrence_change <- function(
	...
	range_core <- delineate_range_core(pred_vect_display, column = 'N_ag_county_sq', ag_core_quant = ag_core_quant)

	# get range of values for plotting
	vars <- c('N_ag_county_ssp245_2041_2070', 'N_ag_county_ssp245_2071_2100', 'N_ag_county_ssp370_2041_2070', 'N_ag_county_ssp370_2071_2100')

	min_val <- Inf
	max_val <- -Inf
	x_sq <- pred_vect_display$N_ag_county_sq[pred_vect_display$focal_region]
	for (var in vars) {
		...
		delta <- x_fut / x_sq
		min_val <- c(min_val, delta[!is.infinite(delta)])
	...
	this_pred_vect$delta <- log10(this_pred_vect[[response_var]] / this_pred_vect$N_ag_county_sq)
