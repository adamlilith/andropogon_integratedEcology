...
library(nimbleHMC) # Bayesian modeling
	...
	data_biomass_mu <- prepare_biomass(formula_biomass = formula_biomass_mu, n_response_curve_values = n_response_curve_values, calib = FALSE)


data <- list(y_biomass = data_biomass_mu$y_biomass)
constants <- list(

	n_biomass = data_biomass_mu$n_biomass,
	x_by_site_biomass = data_biomass_mu$x_by_site_biomass,
	n_pheno_sites = data_biomass_mu$n_pheno_sites,
	site_index_biomass = data_biomass_mu$site_index_biomass,

	n_counties = data_biomass_mu$n_counties,

	n_terms_biomass_mu = 2
)

inits <- list(
	beta_biomass_mu = c(-2, 2),
	site_biomass_plant_sigma_log = 1,
	site_biomass_mean_sigma = 1,
	log_site_biomass_mu = rep(1, 26),
	y_biomass_sim = data_biomass_mu$y_biomass
...
code <- nimbleCode({
	...
	for (i in 1:n_pheno_sites) {
		...
		log_site_biomass_mu_mean[i] <- inprod(beta_biomass_mu[1:n_terms_biomass_mu], x_by_site_biomass[i, 1:n_terms_biomass_mu])
