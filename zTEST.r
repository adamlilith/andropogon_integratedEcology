...
### Erica Newman | Adam B. Smith | Missouri Botanical Garden | adam.smith@mobot.org | 2024-01
###
### source('C:/Ecology/Drive/Research/Andropogon/Andropogon/andropogon_integratedEcology/sdm_01b_nonintegrated_model_poisson.r')
### source('E:/Adam/Research/Andropogon/Andropogon/andropogon_integratedEcology/sdm_01b_nonintegrated_model_poisson.r')
###
### CONTENTS ###
### setup ###
...
predictor_names <- c('bio2', 'bio5', 'bio7', 'bio12', 'pet_warmest_quarter_mm', 'gdd_5_deg', 'climatic_moisture_index')
...
### FAKE HEIGHT
height_fake_cm <- rgamma(nrow(ag_vect_focus), 3 * ag_vect_focus$bio5 + -1 * ag_vect_focus$bio5, 0.4)
...
### PCA on predictors
pca <- prcomp(ag_focus[ , predictor_names], center = TRUE, scale. = TRUE)
x_pcs <- predict(pca, ag_focus)
x_scaled <- scale(x_pcs)
centers <- attributes(x_scaled)$`scaled:center`
scales <- attributes(x_scaled)$`scaled:scale`

### construct model matrix
form <- paste0('~ 1 + ',
	paste0('PC', seq_along(predictor_names), sep = '', collapse = ' + '),
	' + ',
	paste('I(PC', seq_along(predictor_names), '^2)', sep = '', collapse = ' + ')
)

for (i in 1:(length(predictor_names) - 1)) {
	for (j in (i + 1):length(predictor_names)) {
		form <- paste0(form, ' + PC', i, ':PC', j)
	}
}
...
model_code <- nimbleCode({
  ...
  beta0_sampling ~ dnorm(0, sd = 10)
  beta_area ~ dnorm(0, sd = 10)
  beta_poaceae ~ dnorm(0, sd = 10)
  ...
  for (i in 1:n_counties) {    # this specifies estimates be made for each county
	...
	# sampling bias
	logit(p[i]) <- beta0_sampling +
		beta_area * log_area_km2_scaled[i] +
		beta_poaceae * log_num_poaceae_records_scaled[i]
...
monitors <- c('beta', 'beta0_sampling', 'beta_area', 'beta_poaceae', 'lambda')
conf <- configureMCMC(
  model_species,
  monitors = monitors,
  print = TRUE,
...
# conf$addSampler(target = c('beta', 'beta_area', 'beta_poaceae'), type = 'NUTS')
...
saveRDS(chains, './analysis/sdm_chains.rds')

# graphing trace plots for all betas
png('./analysis/sdm_beta_trace.png', width = 1800, height = 1200)
pars <- paste0('beta[', 1:n_sdm_terms, ']')
print(mcmc_trace(chains$samples, pars = pars))
dev.off()

# graphing trace plots for all "extra" betas
png('./analysis/sdm_beta_extra_trace.png', width = 1500, height = 800)
pars <- c('beta0_sampling', 'beta_poaceae', 'beta_area')
print(mcmc_trace(chains$samples, pars = pars))
dev.off()

# graphing density plots for all betas
png('./analysis/sdm_beta_density.png', width = 1800, height = 1200)
pars <- paste0('beta[', 1:n_sdm_terms, ']')
print(mcmc_dens_overlay(chains$samples, pars = pars))
dev.off()

# graphing trace plots for all "extra" betas
png('./analysis/sdm_beta_extra_density.png', width = 2000, height = 800)
pars <- c('beta0_sampling', 'beta_poaceae', 'beta_area')
print(mcmc_dens_overlay(chains$samples, pars = pars))
dev.off()
...
png('./analysis/sdm_lambda.png', width = 1200, height = 1000, res = 300)
...
png('./analysis/sdm_lambda_90ci_vs_mean.png', width = 1200, height = 1000, res = 300)
